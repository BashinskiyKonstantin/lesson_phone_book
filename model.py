...
def create_phone_book(): ############# Создание нового справочника
    with open (f"{input("Введите имя справочника: ")}.txt", "a", encoding='UTF-8') as file:
        file.close()
...
def add_new_contanct(new_contact: list[str]):
    global phone_book
...
def eddit_contact(u_id: int, edited_contact:list[str]):
    global phone_book
